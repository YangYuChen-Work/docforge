...
The frontend fallback citation-state derivation must consult persisted citation state and citations before treating a chapter as generically generating. Any provider response containing an out-of-scope source ID must never produce `explicit` rows; only matched excerpts that were actually sent in this generation may be persisted as `context`, while model-only quote text without matched context is discarded and accompanied by a missing-information warning. Failed chapters need a dedicated `failed` source-panel state instead of being presented as ordinary missing sources. Preparation and post-processing failures must also leave the chapter in `failed` without committing final provenance rows.
